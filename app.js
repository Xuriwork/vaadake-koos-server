...
const origins = [
	'https://vaadakekoos.web.app',
	'vaadakekoos.firebaseapp.com',
	'http://localhost:3000',
];
...
io.on('connection', (socket) => {
	...
	socket.on(VERIFY_PASSCODE, ({ roomName, passcode }, callback) => {
		...
		bcrypt.compare(passcode, room.passcode).then((result) => {
	...
	socket.on(SYNC_BUTTON, (callback) => {
		const room = getRoomByName(socket.roomName);
		io.sockets.connected[room.host].emit(
			GET_HOST_TIME_AND_PLAYER_STATE,
			({ currentTime, playerState }) => callback({ currentTime, playerState })
		);
	});
	...
	socket.on(CHANGE_VIDEO, (videoURL) => {
		if (requestIsNotFromHost(socket)) {
			return sendClientUnsuccessNotification(
	...
	socket.on(CHANGE_ROOM_ID, (newRoomId, callback) => {
		...
		io.in(room.name).emit('GET_ROOM_INFO', {
	...
	socket.on(SET_ROOM_PASSCODE, (passcode) => {
		...
		if (passcode.length > 50) {
			return sendClientUnsuccessNotification(
				'Room Passcode can only be up to 50 characters'
			);
		}
